...
EOF
    ...
    # Istio sidecar is killed if the container Completes or is in Error - this allows the job pod to Complete.
    else
        echo "Found pod $pod with restartPolicy '$restartpolicy' and container '$name' with status '$status'"
        if [ "$name" != "istio-proxy" ] && [ "$status" != "Completed" ] && [ "$status" != "Error" ]; then
            echo "$name is not yet completed";
            all_containers_completed="false"
        fi;
    fi;
